--Look in Server/ databases/ System Databases/ master/ Tables  for these tables once created
USE [master]
GO
...
SELECT        Customers.cust_id, Customers.cust_name, Customers.cust_address, Customers.cust_city, Customers.cust_state, Customers.cust_zip, Customers.cust_country, Customers.cust_contact, Customers.cust_email, 
                         Orders.order_num, Orders.order_date, Orders.cust_id AS Expr1, OrderItems.order_num AS Expr2, OrderItems.order_item, OrderItems.prod_id, OrderItems.quantity, OrderItems.item_price
FROM            OrderItems INNER JOIN
                         Orders ON OrderItems.order_num = Orders.order_num RIGHT OUTER JOIN
...
Select prod_id, prod_name, prod_price from products  --note still works with missing end character
Select * from Products;
SELECT vend_id FROM Products;
SELECT Distinct vend_id from Products;
SELECT prod_name FROM Products FETCH FIRST 5 ROWS ONLY;  --DB2 syntax
SELECT prod_name FROM Products LIMIT 5;	--MySQL, Postgre, SQLLite syntax
SELECT prod_name FROM Products LIMIT 5 OFFSET 5;  --not sure where this works
# A comment  --less commonly supported (not in MS SQL)
...
FROM Products
ORDER BY prod_price, prod_name;	--using column name

SELECT prod_id, prod_price, prod_name
FROM products
ORDER BY 2, 3;		--using column position

/*sort direction*/
SELECT prod_id, prod_price, prod_name
FROM Products
ORDER BY prod_price DESC;  --or DESC,    ASC is the default order

--Multiple sorts one default other not
SELECT prod_id, prod_price, prod_name
FROM Products
